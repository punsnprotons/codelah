...
type ModuleScreenProps = { module: ModuleSpec; index: number; priorSource: string; onContinue: (source: string) => void };
function ModuleScreen({ module, index, priorSource, onContinue }: ModuleScreenProps) {
...
export function App() {
  ...
  if (screen === 'module') return <ModuleScreen index={moduleIndex} key={moduleIndex} module={modules[moduleIndex]} onContinue={(source) => { setCompletedSources((current) => [...current, source]); if (moduleIndex === modules.length - 1) setScreen('assembly'); else setModuleIndex((current) => current + 1); }} priorSource={priorSource} />;
