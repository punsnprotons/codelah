...
function DiagnosticScreen({ onBack, onContinue }: { onBack: () => void; onContinue: () => void }) {
  ...
  const answers = [
    { label: 'A number', visual: '12', visualClass: 'answer-card__visual--number' },
    { label: 'Text', visual: '“12”', visualClass: 'answer-card__visual--text' },
    { label: 'A calculation', visual: '12 + 3', visualClass: 'answer-card__visual--calculation' },
  ];

  return <main className="dark-screen diagnostic-screen"><Progress screen="diagnostic" /><section className="diagnostic-content" aria-labelledby="diagnostic-title"><p className="diagnostic-step">Quick check</p><h1 id="diagnostic-title">When someone types <code>12</code>, what does <code>input()</code> give Python?</h1><p className="diagnostic-context">Think about what reaches the program before it converts anything.</p><div className="answer-grid" role="radiogroup" aria-label="Concept check answers">{answers.map((option) => <button aria-checked={answer === option.label} className={`answer-card ${answer === option.label ? 'answer-card--selected' : ''}`} key={option.label} onClick={() => { setAnswer(option.label); setChecked(false); }} role="radio" type="button"><span className={`answer-card__visual ${option.visualClass}`}>{option.visual}</span><span>{option.label}</span></button>)}</div>{checked && <div className={`feedback ${isCorrect ? 'feedback--success' : ''}`} role="status">{isCorrect ? 'Exactly. Input begins as text; you decide when to turn it into a number.' : 'Think about what a keyboard sends before the program converts it.'}</div>}</section><div className="diagnostic-actions"><button aria-label="Go back" className="journey-back" onClick={onBack} type="button">Back</button>{checked && isCorrect ? <button className="primary-button bottom-button" onClick={onContinue} type="button">Plan the program</button> : <button className="primary-button bottom-button" disabled={!answer} onClick={() => setChecked(true)} type="button">Check answer</button>}</div></main>;
}
