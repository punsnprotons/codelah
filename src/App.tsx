...
function ModuleScreen({ module, index, priorSource, onContinue }: ModuleScreenProps) {
  ...
  const source = `${priorSource}${priorSource ? '\n\n' : ''}${code}`; const lineCount = Math.max(1, code.split('\n').length); const complete = state.status === 'passed'; const activeBlock = planBlocks[index];
  return <main className="light-screen module-screen"><Progress screen="module" /><section className="module-title"><p className="module-kicker">Step {index + 1} of {modules.length}</p><h1>Turn “{activeBlock.label}” into Python.</h1><p>Make this one plan block real code.</p></section><div className="module-workspace"><ol aria-label="Pseudocode plan" className="plan-sequence">{planBlocks.map((block, blockIndex) => <li aria-current={blockIndex === index ? 'step' : undefined} className={`plan-sequence__block plan-sequence__block--${block.tone} ${blockIndex === index ? 'plan-sequence__block--active' : ''}`} key={block.label}><span>{block.label}</span>{blockIndex === index && <b>Now</b>}</li>)}</ol><section className="module-focus" aria-labelledby="current-block-title"><div className={`current-plan-block current-plan-block--${activeBlock.tone}`}><p>Current plan block</p><strong id="current-block-title">{activeBlock.label}</strong></div><p className="module-guidance">{module.hint}</p><label className="code-panel"><span className="sr-only">Python code for the current module</span><span aria-hidden="true" className="line-number">{Array.from({ length: lineCount }, (_, line) => line + 1).join('\n')}</span><textarea aria-label="Python code for the current module" autoComplete="off" name="python-code" spellCheck="false" value={code} onChange={(event) => setCode(event.target.value)} /></label><div className="module-check"><span>Test input</span><code>{module.inputs.join(' · ')}</code>{state.status === 'loading' && <p>Preparing Python…</p>}{state.status === 'running' && <p>Checking your block…</p>}{state.status === 'passed' && <p className="module-check--success" role="status">✓ Block complete{state.output.length > 0 ? ` · ${state.output.join(' ')}` : ''}</p>}{state.status === 'failed' && <p className="module-check--error" role="status">{state.message}</p>}{state.status === 'timed_out' && <p className="module-check--error" role="status">That took too long. The runner was reset safely.</p>}</div></section></div><div className="module-actions">{complete ? <button className="primary-button" onClick={() => onContinue(code)} type="button">{index === modules.length - 1 ? 'Assemble the program' : 'Write the next block'}</button> : <button className="primary-button" disabled={state.status === 'loading' || state.status === 'running'} onClick={() => run(source, module.inputs, module.assertion, module.failureMessage)} type="button">Check this block</button>}</div></main>;
}
