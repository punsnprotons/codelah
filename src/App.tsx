...
type ModuleSpec = { title: string; planStep: string; hint: string; why: string; starter: string; inputs: string[] };
const modules: ModuleSpec[] = [
  { title: 'Turn text into a number', planStep: 'Convert the first score to a number', hint: 'Your program receives text. Convert it before calculating.', why: 'This makes the first value safe to use in math.', starter: 'first_score = float(input("Enter the first score: "))', inputs: ['4'] },
  { title: 'Collect the second value', planStep: 'Get the second score as a number', hint: 'Use the same conversion pattern for the second value.', why: 'A calculator needs two numeric values before it can compare or combine them.', starter: 'second_score = float(input("Enter the second score: "))', inputs: ['4', '3'] },
  { title: 'Ask what to calculate', planStep: 'Choose an operation', hint: 'Keep the operation as text so you can compare it to +, -, *, and /.', why: 'The operation tells your program which calculation to use.', starter: 'operation = input("Choose an operation (+, -, *, /): ")', inputs: ['4', '3', '*'] },
  { title: 'Protect division by zero', planStep: 'Check the denominator before division', hint: 'Write an if statement that only runs when the operation is / and the second score is 0.', why: 'A helpful message keeps a bad calculation from becoming a crash.', starter: 'if operation == "/" and second_score == 0:\n    message = "Error: You cannot divide by zero!"', inputs: ['4', '0', '/'] },
  { title: 'Calculate the result', planStep: 'Calculate from the chosen operation', hint: 'Use if / elif branches. Start with the multiplication case used by the test.', why: 'Branches let one program handle several valid choices clearly.', starter: 'if operation == "+":\n    result = first_score + second_score\nelif operation == "-":\n    result = first_score - second_score\nelif operation == "*":\n    result = first_score * second_score\nelif operation == "/":\n    result = first_score / second_score\nelse:\n    result = None', inputs: ['4', '3', '*'] },
];

const planBlocks = [
  { label: 'Get first score', tone: 'indigo' },
  { label: 'Get second score', tone: 'blue' },
  { label: 'Choose an operation', tone: 'teal' },
  { label: 'Check for division by zero', tone: 'lime' },
  { label: 'Show result or helpful error', tone: 'orange' },
];
...
type ModuleScreenProps = { module: ModuleSpec; index: number; onContinue: () => void };
function ModuleScreen({ module, index, onContinue }: ModuleScreenProps) {
  const [code, setCode] = useState(''); const { state, run } = usePythonRunner();
  useEffect(() => setCode(''), [module]);
  const lineCount = Math.max(1, code.split('\n').length); const complete = state.status === 'passed'; const activeBlock = planBlocks[index]; const nextBlock = planBlocks[index + 1];
  const nextInstruction = nextBlock ? `Pass this check to unlock “${nextBlock.label}”.` : 'Pass this check to assemble the complete program.';
  return <main className="light-screen module-screen"><Progress screen="module" /><div className="module-workspace"><aside className="module-sidebar"><section className="module-title"><p className="module-kicker">Step {index + 1} of {modules.length}</p><h1>Turn “{activeBlock.label}” into Python.</h1></section><ol aria-label="Pseudocode plan" className="plan-sequence">{planBlocks.map((block, blockIndex) => <li aria-current={blockIndex === index ? 'step' : undefined} className={`plan-sequence__block plan-sequence__block--${block.tone} ${blockIndex === index ? 'plan-sequence__block--active' : ''}`} key={block.label}><span>{block.label}</span>{blockIndex === index && <b>Now</b>}</li>)}</ol></aside><section className="module-focus" aria-labelledby="current-block-title"><div className={`current-plan-block current-plan-block--${activeBlock.tone}`}><p>Current plan block</p><strong id="current-block-title">{activeBlock.label}</strong></div><p className="module-guidance">{module.hint}</p><label className="code-panel"><span className="sr-only">Python code for the current module</span><span aria-hidden="true" className="line-number">{Array.from({ length: lineCount }, (_, line) => line + 1).join('\n')}</span><textarea aria-label="Python code for the current module" autoComplete="off" name="python-code" placeholder="Write the Python for this block here…" spellCheck="false" value={code} onChange={(event) => setCode(event.target.value)} /></label><div className="module-check"><span>Test input</span><code>{module.inputs.join(' · ')}</code>{state.status === 'loading' && <p>Preparing Python…</p>}{state.status === 'running' && <p>Checking your block…</p>}{state.status === 'passed' && <p className="module-check--success" role="status">✓ Python ran successfully{state.output.length > 0 ? ` · ${state.output.join(' ')}` : ''}</p>}{state.status === 'failed' && <p className="module-check--error" role="status">{state.message}</p>}{state.status === 'timed_out' && <p className="module-check--error" role="status">That took too long. The runner was reset safely.</p>}{state.status === 'idle' && <p className="module-check__journey">{nextInstruction}</p>}</div></section></div><div className="module-actions">{complete ? <button className="primary-button" onClick={onContinue} type="button">{index === modules.length - 1 ? 'Assemble the program' : `Write “${nextBlock?.label}”`}</button> : <button className="primary-button" disabled={state.status === 'loading' || state.status === 'running'} onClick={() => run(code, module.inputs)} type="button">Run this block</button>}</div></main>;
}
...
export function App() {
  const [screen, setScreen] = useState<Screen>('interests'); const [domains, setDomains] = useState<string[]>([]); const [moduleIndex, setModuleIndex] = useState(0);
  const context = lessonContexts[domains[0] ?? 'sports'];
  const restart = () => { setDomains([]); setModuleIndex(0); setScreen('interests'); };
  ...
  if (screen === 'module') return <ModuleScreen index={moduleIndex} key={moduleIndex} module={modules[moduleIndex]} onContinue={() => { if (moduleIndex === modules.length - 1) setScreen('assembly'); else setModuleIndex((current) => current + 1); }} />;
