...
This manual check is **deferred from M1**. Run it on the intended browser/device before an institutional education release or any release that collects learner data; it is not required for the zero-data 18+ public-preview infrastructure phase.
...
**Deferred acceptance action:** a tester with VoiceOver/NVDA access must complete the two manual journeys above on the intended browser/device and fill in the required environment and result details before an institutional education release or any release that collects learner data.
